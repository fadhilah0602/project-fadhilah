...
@if (session('message_delete'))
    <div x-data="{ isShow: true}"  x-show="isShow" class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
        <strong class="font-bold">{{ session('message_delete') }}</strong>
        <span class="block sm:inline"></span>
        <span class="absolute top-0 bottom-0 right-0 px-4 py-3" @click="isShow = false" >
...
@include('sub.message_error',['message'=>session('message_del')])
    {{-- <div class="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
        <strong class="font-bold">{{ session('message_del') }}</strong>
        <span class="block sm:inline"></span>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <td class="px-4 py-4 whitespace-nowrap">
                ...
                <div class="flex-inline">
                    <a href=" {{ route('bagian.edit',['id'=>$val->id]) }}" >
                      ...
                      <a href=" {{ route('bagian.show',['id'=>$val->id]) }}">
                      ...
                      <a href=" {{ route('bagian.delete',['id'=>$val->id]) }}">
...
{{-- @extends('layouts.app')
@section('content')
 <a href=" {{ route('bagian.create') }}">Tambah Data</a>
<table  cellpadding="1" cellspacing="1" width="100%" class="auto bg-emerald-200 w-16 md:w-32 lg:w-48"  >
    <tr>
...
<tbody>
    ...
    @foreach ($bagian as $val)
        <tr>
            <td>{{ $no }}</td>
            <td>{{ $val->name }}</td>
            <td><a href=" {{ route('bagian.edit',['id'=>$val->id]) }}"><button class="text-white py-1 px-2 bg-green-500 hover:bg-green-700 rounded-md">Edit</button></a>
                <a href=" {{ route('bagian.show',['id'=>$val->id]) }}"><button class="text-white py-1 px-2 bg-gray-500 hover:bg-gray-700 rounded-md">View</button></a>
                <a href=" {{ route('bagian.delete',['id'=>$val->id]) }}"><button class="text-white py-1 px-2 bg-yellow-500 hover:bg-yellow-700 rounded-md">Delete</button></a>
            </td>
        </tr>
    <?php $no++ ;?>
    @endforeach
...
<a href=" {{ route('bagian.create') }}" >
<button class="flex ml-auto text-white bg-blue-500 border-0 py-2 
